...
function complete(){
    $('.saving').hide();
     ...
     $.post("http://ai.khichuri.net/save.php",
    {
        poem: fullpoem,
        title: title,
    },
    function(data, status){
         $('#poem').append("Link: <a href='http://ai.khichuri.net/poem.php?id="+data+"'>http://ai.khichuri.net/poem.php?id="+data+"</a>");
    });
...
function One(word){
    ...
    $.get("http://ai.khichuri.net/poemwrite.php?word="+word, function(data, status){
           // $('#poem').append(data+" ");
        makeline += data+" ";
        if(poemline < 5){
            Two(data);
